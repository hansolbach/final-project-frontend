...
function draw(error, us, maptemplate) {
  ...
  d3.select(".g-hed").text("Statewide Emissions Data");

  //Appends chart intro text
  d3.select(".g-intro").text("State carbon dioxide emissions in 2014, millions of metric tons");
  ...
  map.append("g")
    ...
    .on("mouseover", function(d) {
      ...
      .style("opacity", 1)
      div.text(stateByFIPS[d.id] + ": " + dataByFIPS[d.id] + " million metric tons, 2014")
  ...
  d3.select(".g-source-reg")
    .text("United States Energy Information Agency")
...
function changeData(csv_filename) {
    ...
    document.querySelector('#maptemplate').onclick = function () {
      changeData("https://raw.githubusercontent.com/hansolbach/backend/master/chart2/maptemplate.csv");
    }

    // Add onclick function to button 2
    document.querySelector('#stateenergy').onclick = function () {
      changeData("https://raw.githubusercontent.com/hansolbach/backend/master/chart2/StateEnergy.csv");
    }